...
namespace LedsGo.Plugin.Knop
{
    ...
    public class DockablePanel : ITapDockPanel
    {
        ...
        public FrameworkElement CreateElement(ITapDockContext context)
        {
            ...
            //Attach custom start/stop/load events to editor.
            EventsTapPlan.StartHandler += (s, a) => GuiHelper.GuiInvoke(() => { planThread = context.Run(); });
            EventsTapPlan.StopHandler += (s, a) => GuiHelper.GuiInvoke(() => { planThread?.Abort(); });
            EventsTapPlan.LoadHandeler += (s, a) => GuiHelper.GuiInvoke(() =>
            {
                context.Plan = TestPlan.Load(EventsTapPlan.sFilePath);
            });


           EventsTapPlan.Listen();
        ...
        class dockResultListener : ResultListener
        {
            ...
            {
                //Verdict verdict = planRun.Verdict;
                EventsTapPlan.InitDone = false;
                if(EventsTapPlan.InitDone == false)
                {
                    EventsTapPlan.PlanRun = false;
                }                            
